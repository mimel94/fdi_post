...
def save_image(img, filename, uploads_path):
    ...
    """
    try:
        imsave((uploads_path, '/'+filename), arr=np.squeeze(img))
        print('-->'+uploads_path)
        flash("Image saved!")
...
def load_embeddings(user):
    ...
    for embedding in glob.iglob(pathname='vision/embeddings/'+str(user)+'/*.npy'):
...
def identify_face(embedding, embedding_dict):
    ...
    try:
        ...
        if min_distance <= 1.1:
            ...
            result = "Nombre: " + str(identity) + ", Distancia: " + str(min_distance)
            status = True
            return result, status

        else:
            result = "No reconocido" + str(min_distance)
            status = False
            return result , status
